...
const migrateSingleFileModel = async (req_body) => {
    ...
    const query = "UPDATE migration_table_files SET new_url = $2 WHERE id = $1;";
    ...
    try {
        ...
        if (writeResponse == "Good") {
            ...
            if (azureResponse != 'Bad') {
                ...
                returnResponse.status = "error";
                returnResponse.message = "Failed to upload file to Azure.";
                return JSON.stringify(returnResponse);
            }
        } else {
            returnResponse.status = "error";
            returnResponse.message = "Failed to download file.";
            return JSON.stringify(returnResponse);
        }
        ...
        returnResponse.message = "File successfully migrated.";
        return JSON.stringify(returnResponse);
    } catch (error) {
        console.log('migrate single file model error says: ', error.message);
    }

}

module.exports = migrateSingleFileModel;